angular.module('hsgc').directive('footballPlayerStats', function() {
    return {
        restrict: 'EA',
        require: '^datacast',
        scope: {
        ...
        link: function(scope, element, attrs, datacastCtrl) {
            ...
            attrs.$observe('teamId', function() {
                if (scope.teamId != '') {
                    scope.$watch(
                        function() {
                            return datacastCtrl.getPlayerStatsForTeam(
                                scope.teamId
                            );
                        },
                        function(oldValue, newValue, sc) {
                            sc.playerStats = newValue;
                            sc.anyPlayerStatsAvailable =
                                newValue &&
                                newValue.passingStats.length +
                                    newValue.rushingStats.length +
                                    newValue.receivingStats.length +
                                    newValue.defensiveStats.length +
                                    newValue.kickingStats.length +
                                    newValue.puntingStats.length +
                                    newValue.puntReturnStats.length +
                                    newValue.kickReturnStats.length >
                                    0;
                        }
                    );
                    ...
                    scope.$watch(datacastCtrl.getStatsAvailable, function(
                        newValue,
                        oldValue,
                        sc
                    ) {
