angular.module('hsgc')
  .directive('fullBoxScore', ['hsgcConfig', '$log', function(config, $log) {
    return {
      restrict: 'AE',
      scope: {
        unityGameKey: "@game",
        ...
        if (typeof(ga) !== "undefined" && config.gaTracker) {
          ...
          scope.$on('datacastLoaded', function() {
              ...
              if (--runCountdown <= 0) {
                ...
                ga('send', 'event', 'heartbeat', 'heartbeat', 'stats', { nonInteraction: true });
                runCountdown = 10;
              } else {
                $log.debug('Skipping GA heartbeat until this countdown is below zero: ', runCountdown);
              }
          });
        }
      },
